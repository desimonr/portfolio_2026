        ...
        <section id="skills" className="py-48 bg-white border-y border-slate relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-3 gap-12">
                {content.categories.map((cat) => (
                    <div key={cat.id} className="group relative pl-6 border-l border-slate hover:border-blurple transition-colors duration-300">
                        ...
                        <ul className="space-y-4 font-medium text-ink/80 text-lg">
                            {cat.items.map((item, idx) => (
                                <li key={idx} className="hover:text-ink transition-colors cursor-default">{item}</li>
                            ))}
                        </ul>
                    </div>
                ))}
